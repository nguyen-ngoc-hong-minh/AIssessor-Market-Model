...
function recommendationContext(strategy: {
  priorities: string[]; budget?: number; budgetAmount?: number; budgetCurrency?: string; usageType: "one_off" | "monthly"; deadline?: string; originalInput: string; expectedResult: string;
  ...
  return {
    ...
    budgetOriginalAmount: strategy.budgetAmount ?? null,
    budgetOriginalCurrency: strategy.budgetCurrency,
    region,
    now: Date.now(),
...
function storedPlan(record: { planType: string; recommendations: unknown; costEstimate: unknown; assumptions: string[]; fullPlan?: unknown }): StrategyPlan | null {
  ...
  return {
    ...
    inputsUsed: {
      projectDescription: null, expectedResult: null, budgetUsd: null, budgetOriginalAmount: null, budgetOriginalCurrency: null, deadline: null, priorityRanking: [], existingTools: [],
