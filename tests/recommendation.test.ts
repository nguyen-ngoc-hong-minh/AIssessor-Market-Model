...
describe("deterministic recommendation engine", () => {
  ...
  it("records every saved answer in the plan's auditable input summary", () => {
    const plan = generateStrategyPlan([step], [model], { ...context, budgetOriginalAmount: 10_000_000, budgetOriginalCurrency: "VND", projectDescription: "Write a launch campaign", expectedResult: "Approved copy", deadline: "2033-06-01", informationSensitivity: "business", commercialUse: true, providersToAvoid: ["Acme"], preferredLanguage: "Vietnamese", expectedOutputs: "DOCX and PDF", existingTools: ["Provider"] }, "recommended");
    expect(plan.inputsUsed).toMatchObject({ budgetOriginalAmount: 10_000_000, budgetOriginalCurrency: "VND", projectDescription: "Write a launch campaign", expectedResult: "Approved copy", preferredLanguage: "Vietnamese", expectedOutputs: "DOCX and PDF", providersToAvoid: ["Acme"], existingTools: ["Provider"] });
  });
  it("does not charge an already-owned subscription against the budget", () => {
    const subscribed = { ...model, accessOptions: [{ ...model.accessOptions![0], productId: "provider-suite", productName: "Provider Suite", planName: "Plus", accessMethod: "product" as const, monthlyPriceUsd: 20 }] };
    const plan = generateStrategyPlan([step], [subscribed], { ...context, budgetUsd: 0, existingTools: ["Provider Suite"] }, "recommended");
    expect(plan.steps[0].selected).not.toBeNull();
    expect(plan.fixedCostUsd).toBe(0);
    expect(plan.totalCostUsd).toBe(0);
  });
